...
public class MainApp extends Application{
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        checklogin = new CheckBox("checkLogin");
       
        VBox vBox1 = new VBox();
        vBox1.setSpacing(10);
        vBox1.setAlignment(Pos.BASELINE_LEFT);
        vBox1.getChildren().addAll(textFieldRequest, checklogin);
    ...
    private class MyEventHandler implements EventHandler<ActionEvent>{
        ...
        public void handle(ActionEvent event) {
            if(event.getSource() == buttonShow){
               ...
               {
                   filterManager.setFilter(new login());
                    Client client = new Client(filterManager);
               textArea.setText(
               client.sendRequest(textFieldRequest.getText()));
               }else{
               textArea.setText("You must verify the login (click on -- checkLogin -- )");
               }
